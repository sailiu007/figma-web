...
export default function DashboardPage() {
  const { language, projects, roles, permissions } = useAppStore();
  ...
  const cards = [
    { key: 'projects', icon: FolderKanban, value: projects.length, color: '#8b5cf6,#6d28d9', path: '/projects' },
    { key: 'totalMembers', icon: Users, value: projects.reduce((s, p) => s + p.members, 0), color: '#7c3aed,#4f46e5', path: '/projects' },
    { key: 'roles', icon: ShieldCheck, value: roles.length, color: '#6366f1,#4f46e5', path: '/permissions?tab=roles' },
    { key: 'permissions', icon: FileCode2, value: permissions.length, color: '#a855f7,#7e22ce', path: '/permissions?tab=users' },
  ] as const;
    ...
    <div className="p-5 h-full overflow-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-4">
        {cards.map(c => {
          const Icon = c.icon;
          return (
            <button key={c.key} onClick={() => navigate(c.path)} className="glass-card p-5 text-left rounded-[26px]">
